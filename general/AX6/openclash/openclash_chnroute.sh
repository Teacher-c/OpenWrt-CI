...
. /usr/share/openclash/log.sh
   ...
   if [ "${PIPESTATUS[0]}" -eq "0" ] && [ -s "/tmp/china_domains.list" ] && [ -s "/tmp/apple_china_domains.list" ] && [ -s "/tmp/MyDirect_domains.list" ] && [ -n "$(cat "/tmp/china_domains.list" |head -1 |grep "server=")" ]; then
